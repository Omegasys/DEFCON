DEF CON PERSONAL PACKING LIST

IDENTIFICATION & MONEY

* Government-issued ID
* Hotel room key
* Wallet
* Credit card (1–2 only)
* Cash ($300–600)
* Emergency contact card
* Health insurance card (optional)

ESSENTIALS

* Comfortable walking shoes
* Backup sweatshirt
* Hoodie/light jacket
* DEF CON team clothing
* Conference badge
* Backpack for essentials 
* dufflebag for swag and purchased items
* Sunglasses
* Hat
* Watch
* Keys

NOTE-TAKING

* Spiral notebook
* Mechanical pencil
* Extra pencil lead
* Pens
* Pencils
* Highlighter
* Sharpies
* Fine-tip permanent marker
* Sticky notes

PRINTED MATERIALS

* DEF CON venue map
* Hotel map
* Las Vegas Strip map
* Conference schedule
* Daily itinerary
* Meetup locations
* Emergency phone numbers
* Family contact sheet
* Beginner technology glossary

TECHNOLOGY

* Temporary phone (GrapheneOS)
* ThinkPad (Qubes OS)
* Laptop charger
* Wired earbuds/headphones
* Encrypted USB flash drive
* Backup encrypted USB flash drive
* SD card X2
* MicroSD card X2
* USB data blocker
* USB-C hub
* USB-C to USB-A adapter
* USB-C to HDMI adapter
* USB-C to Ethernet adapter
* Ethernet cable
* Portable SSD (optional)
* USB Rubber Ducky (authorized use only)
* WiFi Pineapple (authorized use only)
* Flipper Zero (authorized use only)
* Faraday bag X10

POWER

* 20,000–30,000 mAh power bank
* USB-C charging cable
* USB-A charging cable
* Lightning cable (if needed)
* Multi-port wall charger
* Power strip
* Extension cord (optional)

NETWORKING

* Business cards
* Contact information cards
* GitHub QR code
* LinkedIn/Mastodon QR code (optional)

TOOLS

* sandwitch bags
* Small flashlight
* Mini screwdriver kit
* Velcro cable ties
* Zip ties
* Small ruler
* Lens/glass cleaning cloth
* Glass cleaning alcohol wipes

HYDRATION & FOOD

* Refillable water bottle
* Electrolyte packets
* Protein bars
* Trail mix
* Nuts
* Beef jerky
* Gum
* Mints
* Cough drops

HEALTH & HYGIENE

* Hand sanitizer
* Lip balm
* Sunscreen
* Tissues
* Pain reliever
* Bandages
* Blister bandages
* Prescription medications
* Deodorant
* Wet wipes

COMFORT

* Comfortable walking shoes
* Extra socks
* Ear plugs
* Small towel (optional)
* Backup sweatshirt
* Swimsuit

SWAG & SOUVENIRS

* Stickers to trade
* Challenge coins (optional)
* Pins
* Badge accessories
* Empty reusable shopping bag

SOFTWARE — INSTALL BEFORE LEAVING

* Offline maps
* PDF reader
* Notes application
* Password manager
* SSH client
* Terminal emulator
* Text editor
* Archive manager
* QR code scanner
* Camera tested
* Device backups completed
* Full disk encryption enabled
* System fully updated

PHONE APPS

* Offline maps
* Camera
* Flashlight
* Notes
* Weather app

SAFETY REMINDERS

* Stay hydrated.
* Lock devices when unattended.
* Use USB data blockers when charging from public ports.
* Don’t connect unknown USB devices.
* Keep badge visible.
* Meet family at predetermined locations if separated.
* Keep sensitive electronics secured when not in use.
* Keep backup devices and storage separate from primary equipment.

FINAL CHECK BEFORE LEAVING

* Government-issued ID
* Wallet
* Hotel reservation information
* Conference badge
* Phone
* Laptop
* Laptop charger
* Power bank
* Power strip
* Headphones
* Faraday bag
* Notebook and writing supplies
* Medication
* Water bottle
* Backpack
